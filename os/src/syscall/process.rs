...
fn copy2buffers(src: &[u8], dest: Vec<&mut [u8]>) {
    let mut beg = 0_usize;
    for buf in dest {
...
unsafe fn copy2app<T>(item: &T, dest: *mut T) {
    let raw = from_raw_parts(
